import { ActualAchievement, ArchiveItem, Status } from './../typesdefinition/interfaces';
import { Challenge } from '../typesdefinition/interfaces';
import { getCurrentTask, getTaskArchive, startNewChallenge } from '../typesdefinition/services';
...
const challenges: Challenge[] = [
  {
    ...
    startDate: new Date(`${new Date().getFullYear()}-${new Date().getMonth() + 1}-${new Date().getDate() - 3}`).getTime(),
    tasksOrder: tasks,
    tasksStatus: [
      { state: ItemState.SUCCESS, updated: new Date(`${new Date().getFullYear()}-${new Date().getMonth() + 1}-${new Date().getDate() - 3}`).getTime() },
      { state: ItemState.PENDING, updated: new Date(`${new Date().getFullYear()}-${new Date().getMonth() + 1}-${new Date().getDate() - 3}`).getTime() },
      { state: ItemState.PENDING, updated: new Date(`${new Date().getFullYear()}-${new Date().getMonth() + 1}-${new Date().getDate() - 3}`).getTime() },
      { state: ItemState.PENDING, updated: new Date(`${new Date().getFullYear()}-${new Date().getMonth() + 1}-${new Date().getDate() - 3}`).getTime() },
      { state: ItemState.PENDING, updated: new Date(`${new Date().getFullYear()}-${new Date().getMonth() + 1}-${new Date().getDate() - 3}`).getTime() },
      { state: ItemState.PENDING, updated: new Date(`${new Date().getFullYear()}-${new Date().getMonth() + 1}-${new Date().getDate() - 3}`).getTime() },
    ...
    achievementsStatus:
      [
        { state: ItemState.SUCCESS, updated: new Date(`${new Date().getFullYear()}-${new Date().getMonth() + 1}-${new Date().getDate()}`).getTime() },
        { state: ItemState.PENDING, updated: new Date(`${new Date().getFullYear()}-${new Date().getMonth() + 1}-${new Date().getDate()}`).getTime() },
        { state: ItemState.PENDING, updated: new Date(`${new Date().getFullYear()}-${new Date().getMonth() + 1}-${new Date().getDate()}`).getTime() },
        { state: ItemState.PENDING, updated: new Date(`${new Date().getFullYear()}-${new Date().getMonth() + 1}-${new Date().getDate()}`).getTime() },
        { state: ItemState.PENDING, updated: new Date(`${new Date().getFullYear()}-${new Date().getMonth() + 1}-${new Date().getDate()}`).getTime() },
      ]
...
describe('getCurrentTask: ', () => {
  ...
  test('should always return tasks[3] from challenge[0]', () => {
    const expected = {
      ...
      state: {
        state: "Pending",
        updated: new Date(`${new Date().getFullYear()}-${new Date().getMonth() + 1}-${new Date().getDate() - 3}`).getTime()
      }
    };
...
describe('getTaskArchive: ', () => {
  ...
  test('should always return 4 tasks with statuses from challenge[0]', () => {
    const expected: ArchiveItem[] = [
      ...
      {
        id: 6161,
        description: "Become a patron for a creator you like on patreon.com or a similar service",
        status: {
